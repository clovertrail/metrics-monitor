// --------------------------------------------------------------------------------------------------------------------
// <copyright file="MetricQueryResponseDeserializer.cs" company="Microsoft Corporation">
            ...
            int numSamplingTypesRequested = -1)
        {
            ...
            using (var reader = new BinaryReader(stream))
            {
                ...
                for (int i = 0; i < numSeries; i++)
                {
                    ...
                    if (definitions == null)
                    {
                        seriesArray[i] = new TimeSeries<MetricIdentifier, double?>(DefaultDateTime, DefaultDateTime, SerializationConstants.DefaultSeriesResolutionInMinutes, null, deserialized.Values, deserialized.ErrorCode);
                    }
                    else
                    {
